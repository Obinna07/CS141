...
// struct to represent each way of a set in the cache
typedef struct set_associative_way
{
    memory_block* mb;       // memory block to hold way data
    bool valid;
    bool dirty;
} set_associative_way;
...
typedef struct set_associative_cache
{
    main_memory* mm;
    cache_stats cs;
    set_associative_way sets[SET_ASSOCIATIVE_NUM_SETS][SET_ASSOCIATIVE_NUM_WAYS];
