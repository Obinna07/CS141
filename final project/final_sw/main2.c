...
float * getTile(float *a, int tile_index_x, int tile_index_y, int in_cols, int numblks) {
  ...
  for (int i = 0; i < numblks; i++) {
    for (int j = 0; j < numblks; j ++ ) {
      new_tile[i * numblks + j] = a[(tile_index_y * numblks * in_cols + i) + (tile_index_x * numblks + j)];
    }
  }
  return new_tile;
}
...
void tileAccumulate(float * a, float * b, float * c, int in_cols, int out_rows, int out_cols, int numblks) {
  ...
  for (int i = 0; i < N; i++) {
    for (int j= 0; j < N; j++) {
      float * a_tile = getTile(a, j, i, out_cols, numblks);
      float * b_tile = getTile(b, j, i, in_cols, numblks);
      float * c_tile = getTile(c, j, i, out_cols, numblks);
      printf("\n");
      print_mat(a_tile, 2, 2);
      printf("============================\n");
      print_mat(b_tile, 2, 2);
      printf("============================\n");
      print_mat(c_tile, 2, 2);
      printf("============================\n");
      ...
      for (int p = 0; p < numblks; p++) {
        for (int q = 0; q < numblks; q ++) {
          ...
          a[a_index] = a[a_index] + a_tile[p * numblks + q];
          printf("%d \n", a_index);
        }
      }
    }
...
int main() {
  ...
  if (log(numblks)/log(2) - round(log(numblks)/log(2)) == 0) {
  ...
  // also correct.
  if ( compare_matrices(mat_a_naive, mat_a_tiled, MATRIX_ROWS, MATRIX_COLS) ) {
